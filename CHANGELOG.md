...
- **Sessions: the Project column wrapped long paths, making some rows three times taller than others.** Path text wrapping at the cell level caused rows containing 216px height against a 77px baseline. It is now a single ellipsed line; the full path is still on hover. Tallest row is now 109px.
