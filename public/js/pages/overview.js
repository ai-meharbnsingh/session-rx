...
import { button, cliIcon, dateText, duration, el, healthNode, metricDelta, number, observedRules, plainText, ruleLabel, severity, sparkline } from '../components/ui.js';
...
const card = (title, value, subtitle, values, delta, tone = 'accent') => {
  const box = el('article', `rx-card summary-card tone-${tone}`);
  box.append(el('h3', '', title), el('strong', 'rx-number', value), el('span', 'rx-label', subtitle));
...
function trendsPreview(trends) {
  const section = el('section', 'rx-card'); const head = el('div','rx-card-head'); head.append(el('div','', 'Trends preview'), el('a','', 'View trends')); head.lastChild.href = '#/trends'; section.append(head);
...
async function renderOverview(mount, data, ctx = {}) {
  ...
  const summaries = el('div','rx-grid rx-grid-four'); summaries.append(card('Sessions analyzed', number(health?.sessionsTotal), 'sessions in this scan', cumulativeSessions, null, 'accent'), card('Problems found', number(rules.length), 'observed rule findings', sessions.map((s) => (s.rules || []).filter((r) => r?.evidence?.status === 'observed').length), null, 'warn'), card('Fixes available', number(fixable), 'observed findings with a fix', sessions.map((s) => (s.rules || []).filter((r) => r?.evidence?.status === 'observed' && r.fix).length), null, 'pass'), card('Not measured', number(unknown), 'checks that could not be measured', sessions.map((s) => s?.score?.unknown || 0), null, 'unknown')); root.append(summaries);
  const healthCard = el('section','rx-card'); const healthHead = el('div','rx-card-head'); healthHead.append(el('h2','', 'Health summary'), el('span','rx-chip', `${unknown} could not be measured`)); healthCard.append(healthHead, donut(health), el('p','local-note', health?.scan?.note || 'Health counts come from the analyzed sessions in this scan.')); root.append(healthCard, trendsPreview(trends));
