...
function issueDistribution(sessions) {
  const counts = new Map();
  observedRules(sessions).forEach(({ rule }) => { const label = ruleLabel(rule); const current = counts.get(label); const tone = severity(rule).toLowerCase(); if (current) current.count += 1; else counts.set(label, { label, count: 1, tone }); });
  const items = [...counts.values()].sort((a, b) => b.count - a.count || a.label.localeCompare(b.label)); const total = items.reduce((sum, item) => sum + item.count, 0); const visible = items.slice(0, 5); if (items.length > 5) visible.push({ label: 'Other', count: items.slice(5).reduce((sum, item) => sum + item.count, 0), tone: 'low' });
  let used = 0; return visible.map((item, index) => { const percent = index === visible.length - 1 ? 100 - used : Math.round(item.count / total * 100); used += percent; return { ...item, percent }; });
}
...
function donut(health, trends) {
  ...
  const stateLine = el('p', 'health-states'); const percentages = largestRemainderPercentages([passed, observed, unknown], total); if (percentages) stateLine.append(el('span', 'state-pass', `${percentages[0]}% passed`), document.createTextNode(' · '), el('span', 'state-observed', `${percentages[1]}% problems found`), document.createTextNode(' · '), el('span', 'state-unknown', `${percentages[2]}% could not be measured`)); else stateLine.textContent = 'Nothing to summarise.'; donutColumn.append(stateLine);
  const distribution = issueDistribution(sessions); const distributionColumn = el('div', 'distribution-column'); distributionColumn.append(el('h3', '', 'Issue distribution'), el('p', 'distribution-note', 'Percentage of all observed findings'));
  if (!distribution.length) distributionColumn.append(el('p', 'not-measured', 'No observed findings in these sessions.'));
  distribution.forEach((item) => { const row = el('div', 'distribution-row'); const label = el('div', 'distribution-label'); const dot = el('span', `distribution-dot ${item.tone}`); dot.setAttribute('aria-hidden', 'true'); label.append(dot, el('span', '', item.label)); row.append(label, el('strong', '', `${item.percent}%`)); const trackBar = el('div', 'bar-track'); const fill = el('div', `bar-fill distribution-fill ${item.tone}`); fill.style.width = `${item.percent}%`; trackBar.append(fill); row.append(trackBar); distributionColumn.append(row); });
...
function lineChart(items, valueKey, tone) {
  const svg = document.createElementNS(svgNs, 'svg'); svg.setAttribute('viewBox', '0 0 280 92'); svg.setAttribute('class', `trend-chart tone-${tone}`); svg.setAttribute('role', 'img');
  ...
  const min = Math.min(...valid); const max = Math.max(...valid); const span = max - min || 1; let segment = [];
  values.forEach((value, index) => { if (value === null) { if (segment.length) { appendLine(svg, segment); segment = []; } return; } segment.push(`${12 + index / Math.max(1, values.length - 1) * 256},${78 - ((value - min) / span) * 58}`); }); if (segment.length) appendLine(svg, segment); return svg;
}

function appendLine(svg, points) { const firstX = points[0].split(',')[0]; const last = points[points.length - 1]; const lastX = last.split(',')[0]; const area = document.createElementNS(svgNs, 'polygon'); area.setAttribute('points', `${points.join(' ')} ${lastX},84 ${firstX},84`); area.setAttribute('class', 'trend-area'); svg.append(area); const poly = document.createElementNS(svgNs, 'polyline'); poly.setAttribute('points', points.join(' ')); poly.setAttribute('fill', 'none'); poly.setAttribute('vector-effect', 'non-scaling-stroke'); svg.append(poly); }
